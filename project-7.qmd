...
This assignment has [starter code](cross_stitch.py), which contains a global constant for a list of symbols to be used and a function to print the pattern called `print_pattern(pattern)`. The started code also imports the library random. Do not make changes to the starter code (changes might cause gradescope test fails).
